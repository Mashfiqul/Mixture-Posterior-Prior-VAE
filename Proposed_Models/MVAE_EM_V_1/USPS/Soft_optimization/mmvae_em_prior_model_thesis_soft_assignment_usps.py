# -*- coding: utf-8 -*-
"""Constrained_MMVAE_EM_Model_Prior_Thesis_Soft_Assignment_USPS.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1KIByTmdFEpd3O2aejysVqbNPA1FXS-5e

# **Modified Mixtures of VAE Model using USPS Dataset**
"""
...
Model="MVAE_EM_Prior"
Assignment="Soft"
Model_name= f"{Model}_{random_seed}_Latent_dim_{latent_dim}_{Assignment}_{Dataset_name}"
...
data_dir = '~/Desktop/USPS_Dataset/data_src'
trainset = datasets.USPS(data_dir, train=True, download=True, transform=t)
testset = datasets.USPS(data_dir, train=False, download=True, transform=t)

print("Training Set Size: ", len(trainset), "\n")
print("Test Set Size: ", len(testset), "\n")
print("Maximum Training Pixel: ", np.max(trainset.data), "\n")
print("Minimum Training Pixel: ", np.min(trainset.data), "\n")
...
y_train=trainset.targets[0:]
y_test=testset.targets[0:]
...
with open(f'Summary_Statistics_Training_{Dataset_name}.txt', 'w') as file:
    ...
    file.write(f"Maximum Training Pixel: {np.max(trainset.data)}\n")
    file.write(f"Minimum Training Pixel: {np.min(trainset.data)}\n")
...
file_path=f"Model_Architecture_{Model_name}.txt"
...
# for training_features,train_labels in train_loader:
...
#   prior_log_likelihood=log_normal_prior_k+log_pi_prior_k
#   q_numerator=torch.exp(prior_log_likelihood)
#   q_denominator=torch.sum(q_numerator, dim=1, keepdim=True)
#   q_y_i_k=q_numerator/q_denominator

#   print("\nPosterior probability Shape:\n", q_y_i_k.shape)
#   print("\nSum along the K axis in posterior probability:\n", torch.sum(q_y_i_k[0,:]))
...
#   best_reconstruction=decoder_mu[torch.arange(decoder_mu.size(0)), best_k_train, :]

#   targets=training_features

#   # Binary Cross Entropy

#   BCE=-F.binary_cross_entropy(input=best_reconstruction, target=targets, reduction="none")
#   log_p_x_z=torch.sum(BCE)


#   # Real-valued Input

#   p_x_z=Normal(loc=best_reconstruction, scale=torch.ones_like(best_reconstruction))
#   MSE=p_x_z.log_prob(targets)
#   log_p_x_z=torch.sum(MSE)

#   ##############################################################################

#   encoder_prior_log_likelihood=(encoder_log_prob+torch.log(q_y_i_k)-log_normal_prior_k-log_pi_prior_k)
#   posterior_times_encoder_prior_log_likelihood=q_y_i_k*encoder_prior_log_likelihood

#   print(encoder_prior_log_likelihood.shape)
#   break
...
for epoch in range(num_epochs):
  ...
  RE_Loss_Train.append(0)
  KL_Loss_Train.append(0)
  Entropy_Loss_Train.append(0)
  num_batches=0
  ...
  for training_features, _ in train_loader:
    ...
    prior_log_likelihood=log_normal_prior_k+log_pi_k

    # Posterior Probability

    q_numerator=torch.exp(prior_log_likelihood)                        # [B, K]
    q_denominator=torch.sum(q_numerator, dim=1, keepdim=True)
    q_y_i_k=q_numerator/q_denominator
    ...
    # q_y_i_k= []
    # for row in prior_log_likelihood:
    ...
    # Binary Input: Binary Cross Entropy
    ...
    log_p_x_z_ik = torch.sum(BCE_Loss, dim=2)

    ##################################### ELBO #################################

    # elbo_ik=log_p_x_z_ik+log_normal_prior_k+log_pi_k-log_q_z_y-torch.log(q_y_i_k)
    # posterior_times_elbo_ik=q_y_i_k*elbo_ik
    # elbo_i=torch.sum(posterior_times_elbo_ik, dim=1)
    # ELBO_Train=torch.sum(elbo_i)


    # Decoder and KL Loss

    RE_Train = torch.sum(q_y_i_k * log_p_x_z_ik)
    KL_Train = torch.sum(q_y_i_k * (log_q_z_y + torch.log(q_y_i_k) - log_normal_prior_k - log_pi_k))
    Entropy_Train=-torch.sum(q_y_i_k*torch.log(q_y_i_k))
    ELBO_Train=RE_Train-KL_Train

    # Total Loss
    Total_Loss = -ELBO_Train
    ...
    RE_Loss_Train[-1] += -RE_Train.item()
    KL_Loss_Train[-1] += -KL_Train.item()
    Entropy_Loss_Train[-1] += -Entropy_Train.item()
  ...
  RE_Loss_Train[-1] /= num_batches
  KL_Loss_Train[-1] /= num_batches
  Entropy_Loss_Train[-1] /= num_batches
  ...
  Best_class=[]
  Test_Loss.append(0)
  RE_Loss_Test.append(0)
  KL_Loss_Test.append(0)
  Entropy_Loss_Test.append(0)
  ...
  with torch.no_grad():
    for test_features, test_labels in test_loader:
      ...
      prior_log_likelihood_test=log_normal_prior_k_test+log_pi_k_test


      ########################### Posterior Probability ########################

      q_numerator_test=torch.exp(prior_log_likelihood_test)                        # [B, K]
      ...
      # q_y_i_k_test= []
      # for row in prior_log_likelihood_test:
      #   q_numerator_test= [decimal.Decimal(value.item()).exp() for value in row]
      #   q_denominator_test = sum(q_numerator_test)
      #   normalized_probability = [value / q_denominator_test for value in q_numerator_test]
      ...
      log_p_x_z_ik_test = torch.sum(BCE_Loss_test, dim=2)


      # elbo_ik_test=log_p_x_z_ik_test+ log_normal_prior_k_test+log_pi_k_test-log_q_z_y_test-torch.log(q_y_i_k_test)
      ...
      RE_Test = torch.sum(q_y_i_k_test * log_p_x_z_ik_test)
      KL_Test = torch.sum(q_y_i_k_test * (log_q_z_y_test + torch.log(q_y_i_k_test) - log_normal_prior_k_test - log_pi_k_test))
      Entropy_Test=-torch.sum(q_y_i_k_test *torch.log(q_y_i_k_test))
      ELBO_Test=RE_Test-KL_Test
      ...
      Test_Loss[-1] += Total_Test_Loss.item()
      RE_Loss_Test[-1] += -RE_Test.item()
      KL_Loss_Test[-1] += -KL_Test.item()
      Entropy_Loss_Test[-1] += -Entropy_Test.item()
      number_of_batches+= 1


    # Average Loss
    Test_Loss[-1] /= number_of_batches
    RE_Loss_Test[-1] /= number_of_batches
    KL_Loss_Test[-1] /= number_of_batches
    Entropy_Loss_Test[-1] /= number_of_batches
    ...
    if (epoch + 1) == num_epochs:
        ...
        plt.axis('off')
        plt.savefig(f'Reconstructed_Digits_{Model_name}.png')
        plt.close()
        ...
        plt.imshow(Reconstructed_Digits)
        plt.axis('off')
        plt.savefig(f'Reconstructed_Mismatch_Digits_{Model_name}.png')
        plt.close()
        ...
        plt.axis('off')
        plt.savefig(f'Original_Random_Images_{Model_name}.png')
        plt.close()
        ...
        plt.axis('off')
        plt.savefig(f'Reconstructed_Random_Images_{Model_name}.png')
        plt.close()

    if (epoch+1)%GMM_interval==0:
        print(f"\n Step: [{epoch+1}/{num_epochs}] \t  Training Loss: {Training_Losses[-1]: 0.2f} \t Test Loss: {Test_Loss[-1]: 0.2f} \
        \t Accuracy (GMM): {Accuracy_GMM[-1]: 0.5f} \t NMI (GMM): {NMI_GMM[-1]: 0.5f} \t ARI (GMM): { ARI_GMM[-1]: 0.5f} \
        \t Accuracy (Posterior): {new_accuracy_posterior: 0.5f}  \t NMI (Posterior): {nmi_posterior: 0.5f}  \t ARI (Posterior): {ari_posterior: 0.5f} \
...
y1=RE_Loss_Train
y2=KL_Loss_Train
y3=Entropy_Loss_Train
y4=Training_Losses
plt.xlabel("Epochs")
plt.ylabel("Negative Loss")
# plt.ylim(-20,250)
plt.plot(x, y1, color="blue", linestyle="solid", linewidth=1, label="RE Loss")
plt.plot(x, y2, color="red", linestyle="dotted", linewidth=1, label="KL Loss")
plt.plot(x, y3, color="green", linestyle="dashdot", linewidth=1, label="Entropy Loss")
plt.plot(x, y4, color="cyan", linestyle="dashed", linewidth=1, label="Training Loss")
plt.legend(loc="best")
plt.grid()
plt.savefig(f"{Model_name}_Losses_Training.pdf", format="pdf", bbox_inches="tight")
plt.close()

"""### **Entropy Loss (Training)**"""

x=np.linspace(1, len(Entropy_Loss_Train), len(Entropy_Loss_Train))
plt.figure(figsize=(8, 6))
plt.xlabel("Epochs")
plt.ylabel("Negative Entropy Loss (Training)")
plt.plot(x, Entropy_Loss_Train, color="blue", linestyle="solid", linewidth=1)
plt.grid()
plt.savefig(f"Negative_Entropy_Loss_Train_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()

"""### **KL Loss (Training)**"""

x=np.linspace(1, len(KL_Loss_Train), len(KL_Loss_Train))
plt.figure(figsize=(8, 6))
plt.xlabel("Epochs")
plt.ylabel("Negative KL Loss (Training)")
plt.plot(x, KL_Loss_Train, color="blue", linestyle="solid", linewidth=1)
plt.grid()
plt.savefig(f"Negative_KL_Loss_Train_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()

"""### **RE Loss (Training)**"""

x=np.linspace(1, len(RE_Loss_Train), len(RE_Loss_Train))
plt.figure(figsize=(8, 6))
plt.xlabel("Epochs")
plt.ylabel("Negative RE Loss (Training)")
plt.plot(x, RE_Loss_Train, color="blue", linestyle="solid", linewidth=1)
plt.grid()
plt.savefig(f"Negative_RE_Loss_Train_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()

"""### **Visualize All Test Losses**"""

plt.figure(figsize=(8,6))
x=np.linspace(1, len(Test_Loss), len(Test_Loss))
y1=RE_Loss_Test
y2=KL_Loss_Test
y3=Entropy_Loss_Test
y4=Test_Loss
plt.xlabel("Epochs")
plt.ylabel("Negative Loss")
# plt.ylim(-20,250)
...
plt.plot(x, y2, color="red", linestyle="dotted", linewidth=1, label="KL Loss")
plt.plot(x, y3, color="green", linestyle="dashdot", linewidth=1, label="Entropy Loss")
plt.plot(x, y4, color="cyan", linestyle="dashed", linewidth=1, label="Test Loss")
plt.legend(loc="best")
plt.grid()
plt.savefig(f"{Model_name}_Losses_Test.pdf", format="pdf", bbox_inches="tight")
plt.close()

"""
### **Entropy Loss (Test)**"""

x=np.linspace(1, len(Entropy_Loss_Test), len(Entropy_Loss_Test))
plt.figure(figsize=(8, 6))
plt.xlabel("Epochs")
plt.ylabel("Negative Entropy Loss (Test)")
plt.plot(x, Entropy_Loss_Test, color="blue", linestyle="solid", linewidth=1)
plt.grid()
plt.savefig(f"Negative_Entropy_Loss_Test_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()

"""### **KL Loss (Test)**"""

x=np.linspace(1, len(KL_Loss_Test), len(KL_Loss_Test))
plt.figure(figsize=(8, 6))
plt.xlabel("Epochs")
plt.ylabel("Negative KL Loss (Test)")
plt.plot(x, KL_Loss_Test, color="blue", linestyle="solid", linewidth=1)
plt.grid()
plt.savefig(f"Negative_KL_Loss_Test_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()

"""### **RE Loss (Test)**"""

x=np.linspace(1, len(RE_Loss_Test), len(RE_Loss_Test))
plt.figure(figsize=(8, 6))
plt.xlabel("Epochs")
plt.ylabel("Negative RE Loss (Test)")
plt.plot(x, RE_Loss_Test, color="blue", linestyle="solid", linewidth=1)
plt.grid()
plt.savefig(f"Negative_RE_Loss_Test_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()

"""### **Store All the Results**"""

hyperparameter_file_path = f'Hyperparameters_{Model_name}.txt'
with open(hyperparameter_file_path, 'w') as file:
  ...
  # file.write(f"--------------------------------------------\n")
  # file.write(f"beta_1 = {beta_1}\n")
  # file.write(f"--------------------------------------------\n")
  # file.write(f"beta_2 = {beta_2}\n")
  file.write(f"==================================================================\n")
  ...
  file.write("\nLoss Functions:\n")
  file.write(f"==================================================================\n")
  file.write(f"Decoder Loss (Training) = {log_p_x_z_ik}\n")
  file.write(f"=============================================================================================\n")
  file.write(f"Encoder Loss (Training) = {q_z_y}\n")
  file.write(f"=============================================================================================\n")
  file.write(f"Decoder Loss (Test) = {log_p_x_z_ik_test}\n")
  file.write(f"=============================================================================================\n")
...
with open(f'Training_Losses_{Model_name}.txt', 'w') as file:
    for loss in Training_Losses:
        file.write(str(loss) + '\n')

with open(f'Test_Losses_{Model_name}.txt', 'w') as file:
  for loss1 in Test_Loss:
    file.write(str(loss1) + '\n')

with open(f'Posterior_Accuracy_New_{Model_name}.txt', 'w') as file:
  for Posterior_Accuracy in Accuracy_Posterior:
    file.write(str(Posterior_Accuracy) + '\n')

with open(f'NMI_Posterior_{Model_name}.txt', 'w') as file:
  for NMI_Posteriors in NMI_Posterior:
    file.write(str(NMI_Posteriors) + '\n')

with open(f'ARI_Posterior_{Model_name}.txt', 'w') as file:
  for ARI_Posteriors in ARI_Posterior:
    file.write(str(ARI_Posteriors) + '\n')

with open(f'Accuracy_GMM_{Model_name}.txt', 'w') as file:
  for loss4 in Accuracy_GMM:
    file.write(str(loss4) + '\n')

with open(f'NMI_GMM_{Model_name}.txt', 'w') as file:
  for loss5 in NMI_GMM:
    file.write(str(loss5) + '\n')

with open(f'ARI_GMM_{Model_name}.txt', 'w') as file:
  for loss6 in ARI_GMM:
    file.write(str(loss6) + '\n')

with open(f'KL_Loss_Training_{Model_name}.txt', 'w') as file:
  for loss10 in KL_Loss_Train:
        file.write(str(loss10) + '\n')

with open(f'RE_Loss_Training_{Model_name}.txt', 'w') as file:
    for loss11 in RE_Loss_Train:
        file.write(str(loss11) + '\n')

with open(f'KL_Loss_Test_{Model_name}.txt', 'w') as file:
  for loss12 in KL_Loss_Test:
        file.write(str(loss12) + '\n')

with open(f'RE_Loss_Test_{Model_name}.txt', 'w') as file:
    for loss13 in RE_Loss_Test:
        file.write(str(loss13) + '\n')
...
results_file_path = f'GMM_Parameters_{Model_name}.txt'
...
results_file_path_2 = f'Model_Parameters_{Model_name}.txt'
...
print(f"\n Accuracy (Posterior): {Accuracy_Posterior[-1]: 0.5f}")
print(f'\n NMI (Posterior): {NMI_Posterior[-1]: 0.5f}')
print(f'\n ARI (Posterior): {ARI_Posterior[-1]: 0.5f}')
print("-"*50)

print("-"*50)
print("\n GMM Results:")
print(f"\n Clustering accuracy (GMM): {Accuracy_GMM[-1]: 0.5f}")
print(f'\n NMI (GMM): {NMI_GMM[-1]: 0.5f}')
print(f'\n ARI (GMM): {ARI_GMM[-1]: 0.5f}')
print("-"*50)
...
text_to_save =f"\n Final Training Loss: {Training_Losses[-1]}\n" \
              ...
              f"\n GMM Results:\n\n"\
              f"\n Accuracy (GMM): {Accuracy_GMM[-1]: 0.5f}\n"\
              f"\n NMI (GMM): {NMI_GMM[-1]: 0.5f}\n"\
              f"\n ARI (GMM): {ARI_GMM[-1]: 0.5f}\n"\
...
file_path = f"Results_{Model_name}.txt"
...
with open(f'Accuracy_Highest_Posterior_{Model_name}.txt', 'w') as file:
...
with open(f'Accuracy_Highest_GMM_{Model_name}.txt', 'w') as file:
...
plt.grid()
plt.savefig(f"SD_Accuracy_Training_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.legend(loc="best")
plt.grid()
plt.savefig(f"Mean_Maximum_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.legend(loc="best")
plt.grid()
plt.savefig(f"Mean_Maximum_{Model_name}_10_epochs.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.legend(loc="best")
plt.grid()
plt.savefig(f"Training_Test_Loss_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.grid()
plt.savefig(f"Negative_ELBO_Training_Loss_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.grid()
plt.savefig(f"Negative_ELBO_Test_Loss_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.legend(loc="best")
plt.grid()
plt.savefig(f"Accuracy_{Model_name}_GMM_Posterior.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.grid()
plt.savefig(f"Accuracy_{Model_name}_Posterior.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.grid()
plt.savefig(f"Accuracy_{Model_name}_GMM.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.grid()
plt.savefig(f"NMI_GMM_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.grid()
plt.savefig(f"NMI_Posterior_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.grid()
plt.savefig(f"ARI_GMM_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.grid()
plt.savefig(f"ARI_Posterior_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.savefig(f"Confusion_Matrix_{Model_name}_GMM.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.ylabel("True Labels")
# plt.title("Confusion Matrix")
plt.savefig(f"Confusion_Matrix_{Model_name}_Posterior.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
df_nn.to_csv(f'Posterior_Probability_NN_{Model_name}.csv', index=False)
...
df_em.to_csv(f'Posterior_Probability_GMM_{Model_name}.csv', index=False)
...
plt.savefig(f"t-SNE_Visualization_{Model_name}_GMM.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.legend(handles=legend_handles, loc='best')
plt.savefig(f"t-SNE_Visualization_{Model_name}_Posterior.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(test_images)
  plt.axis('off')
  plt.savefig(f'Original_Images_{Model_name}.png')
  plt.close()
...
for k in range(num_components):
    ...
    with torch.no_grad():
        ...
        plt.axis('off')
        plt.savefig(f'Reconstructed_Images_{Model_name}_k_{k}.png')
        plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(reconstructions)
  plt.axis('off')
  plt.savefig(f'Reconstructed_Images_{Model_name}.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_Std_1.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_Std_0.5.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_Std_1.5.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_Mean_Negative_0.5.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed}.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed+2}.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_Updated_{random_seed+4}.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed+6}_GMM.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed+8}_GMM.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed+6}_GMM_diag.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed+8}_GMM_diag.png')
  plt.close()
...
legend_handles = [mpatches.Patch(color=cmap(i), label=f'Cluster {i}') for i in np.unique(Updated_GMM_Labels[-1])]
plt.legend(handles=legend_handles, loc='best')
plt.savefig(f"PCA_Visualization_{Model_name}.pdf", format="pdf", bbox_inches="tight")
plt.close()
...
plt.imshow(new_image)
plt.axis('off')
plt.savefig(f'Generated_Image_{Model_name}_{random_seed+8}_PCA_Unmatched.png')
plt.close()
...
plt.imshow(new_image)
plt.axis('off')
plt.savefig(f'Generated_Image_{Model_name}_{random_seed+8}_PCA_Matched.png')
plt.close()
...
plt.imshow(new_image)
plt.axis('off')
plt.savefig(f'Generated_Image_{Model_name}_{random_seed+8}_PCA_Matched_2.png')
plt.close()
