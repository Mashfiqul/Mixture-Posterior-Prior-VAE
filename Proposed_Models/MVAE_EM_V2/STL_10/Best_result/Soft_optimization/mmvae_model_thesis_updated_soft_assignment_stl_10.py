...
# **Modified Mixtures of VAE Model using STL-10 Dataset**
"""
...
for epoch in range(num_epochs):
  ...
  for training_features, _ in train_loader:
    ...
    elbo_ik=log_p_x_z_ik+ prior_posterior_log_likelihood-torch.log(q_y_i_k)
    posterior_times_elbo_ik=q_y_i_k*elbo_ik
    elbo_i=torch.sum(posterior_times_elbo_ik, dim=1)
    ELBO=torch.sum(elbo_i)
  ...
  with torch.no_grad():
    for test_features, test_labels in test_loader:
      ...
      elbo_ik_test=log_p_x_z_ik_test+ prior_posterior_log_likelihood_test-torch.log(q_y_i_k_test)
      posterior_times_elbo_ik_test=q_y_i_k_test*elbo_ik_test
      elbo_i_test=torch.sum(posterior_times_elbo_ik_test, dim=1)
      ELBO_test=torch.sum(elbo_i_test)
