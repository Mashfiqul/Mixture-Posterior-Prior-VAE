# -*- coding: utf-8 -*-
"""MMVAE_Model_Thesis_Soft_Assignment_MNIST_Beta_Factor_Final.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1qg6sJMysx0J53ELOH63FG62uH8PqzvVs
...
Model="MMVAE"
Assignment="Soft"
Model_name= f"{Model}_{random_seed}_Latent_dim_{latent_dim}_{Assignment}_Optimization_{initial_beta_1}_Beta_{beta_1}_{Dataset_name}_dataset"
...
with torch.no_grad():
  ...
  for i in range(gmm_mu.shape[0]):
    ...
    for sample in component_samples:
      sample=sample.reshape(1,-1)
      distance = MD(sample, gmm_mu[i], gmm_covar[i])  # Compute distance for each sample
      all_samples.append((sample, distance.item()))

    # Sort all samples based on distance
    all_samples.sort(key=lambda x: x[1])
    selected_samples = []
    for idx, (sample, distance) in enumerate(all_samples):
        if idx % 5 == 0:
            selected_samples.append(sample)
        if len(selected_samples) == n_smallest_distances:
            break
    all_smallest_samples.extend(selected_samples)
  all_smallest_samples = torch.cat(all_smallest_samples, dim=0)
  z = all_smallest_samples.to(device=device, dtype=torch.float32)
  new_image = model.decoder(z)
  new_image = new_image.view(new_image.size(0), 1, pixel_size[0], pixel_size[1])
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed}_GMM_MD_5.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed}_GMM_MD_10.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed}_GMM_MD_50.png')
  plt.close()
...
with torch.no_grad():
  ...
  plt.imshow(new_image)
  plt.axis('off')
  plt.savefig(f'Generated_Image_{Model_name}_{random_seed}_GMM_MD_100.png')
  plt.close()
