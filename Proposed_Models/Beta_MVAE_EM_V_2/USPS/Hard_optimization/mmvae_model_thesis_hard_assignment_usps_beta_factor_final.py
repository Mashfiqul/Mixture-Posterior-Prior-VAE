# -*- coding: utf-8 -*-
"""MMVAE_Model_Thesis_Hard_Assignment_USPS_Beta_Factor_Final.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1qw8mygppYNN7nkynTVJW99dStLODrYd0
...
Model="MMVAE"
Assignment="Hard"
Model_name= f"{Model}_random_seed_{random_seed}_Latent_dim_{latent_dim}_{Assignment}_Optimization_Beta_{beta_1}_{Dataset_name}_dataset"
...
#   prior_posterior_log_likelihood=log_normal_prior_k+log_pi_prior_k-encoder_log_prob
#   q_numerator=torch.exp(prior_posterior_log_likelihood)
...
for epoch in range(num_epochs):
  ...
  for training_features, _ in train_loader:
    ...
    q_y_i_k= []
    for row in prior_posterior_log_likelihood:
    ...
    best_k_train=torch.argmax(q_y_i_k, dim=1)
    RE_i_Train = log_p_x_z_ik[range(batch_size), best_k_train]
    RE_Train=torch.sum(RE_i_Train)
    KL_i_Train = (log_q_z_y - log_normal_prior_k - log_pi_k)[range(batch_size), best_k_train]
    KL_Train = torch.sum(KL_i_Train)
    ELBO_Train=RE_Train-beta_1*KL_Train
  ...
  with torch.no_grad():
    for test_features, test_labels in test_loader:
      ...
      q_y_i_k_test= []
      for row in prior_posterior_log_likelihood_test:
      ...
      RE_i_Test = log_p_x_z_ik_test[range(batch_size), best_k_test]
      RE_Test = torch.sum(RE_i_Test)
      KL_i_Test = (log_q_z_y_test - log_normal_prior_k_test - log_pi_k_test)[range(batch_size), best_k_test]
      KL_Test = torch.sum(KL_i_Test)
      ELBO_Test=RE_Test-beta_1*KL_Test
...
plt.plot(x, y3, color="green", linestyle="dashdot", linewidth=1, label="Training Loss")
plt.legend(loc="best")
plt.grid()
...
plt.plot(x, y1, color="blue", linestyle="solid", linewidth=1, label="RE Loss")
plt.plot(x, y2, color="red", linestyle="dotted", linewidth=1, label="KL Loss")
plt.plot(x, y3, color="green", linestyle="dashdot", linewidth=1, label="Test Loss")
plt.legend(loc="best")
plt.grid()
...
Selected_Accuracy=Accuracy_Posterior[::50]
plt.figure(figsize=(8, 6))
x=np.linspace(50, len(Accuracy_GMM)*50, len(Accuracy_GMM))
y=np.linspace(50, len(Selected_Accuracy)*50, len(Selected_Accuracy))
...
x=np.linspace(50, len(NMI_GMM)*50, len(NMI_GMM))
plt.figure(figsize=(8, 6))
plt.xlabel("Epochs")
...
x=np.linspace(50, len(ARI_GMM)*50, len(ARI_GMM))
plt.figure(figsize=(8, 6))
plt.xlabel("Epochs")
